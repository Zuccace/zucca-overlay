...
#	enable_feature USE_SYSTEM_LIB_LUA
	...
	use verbose_compilation && enable_feature VERBOSE
...
src_install() {
	...
	function mess_install() {
		...
		newman "${S_MANDIR}mess.6" sdlmess.6
		doman "${S_MANDIR}mess.6"
	}
	if use arcade ; then
		if use mess ; then
			MAMEBIN="mame${suffix}"
		...
		doman "${S_MANDIR}mame.6"
		newman "${S_MANDIR}mame.6" ${PN}.6
	elif use mess ; then
		MAMEBIN="mess${suffix}"
		mess_install
	fi
